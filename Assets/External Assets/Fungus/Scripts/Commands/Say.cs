...
namespace Fungus
{
    ...
    public class Say : Command, ILocalizable
    {
        ...
        [Tooltip("Voiceover addressable")]
        [SerializeField] protected AssetReference voiceOverClipAddress;
        ...
        public override void OnEnter()
        {
            ...
            StartCoroutine(SayRoutine(subbedText));        
        }

        private IEnumerator SayRoutine(string subbedText)
        {
            yield return null;
            ...
            if (voiceOverClipAddress.RuntimeKeyIsValid())
            {
                _loadHandle = voiceOverClipAddress.LoadAssetAsync<AudioClip>();
                while (!_loadHandle.IsDone)
                {
                    yield return new WaitForEndOfFrame();
                }
                var addressableClip = _loadHandle.Result;


                sayDialog.SetActive(true);
                sayDialog.Say(subbedText, !extendPrevious, waitForClick, fadeWhenDone, stopVoiceover, waitForVO, addressableClip, delegate {
                    Continue();
                });
            }
            else
